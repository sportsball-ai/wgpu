...
impl<S: Surface + DynResource> DynSurface for S {
    ...
    unsafe fn discard_texture(&self, mut texture: Box<dyn DynSurfaceTexture>) {
        let texture = texture.as_mut().expect_downcast_mut();
        unsafe { S::discard_texture(self, texture) }
    }
}
